...
void ImageTransform::grayscale()
{
    ...
    for (uint32_t y = 0; y < cfg->h; y++)
    {
        ...
        {
            auto p = getPixel(x, y);
            uint8_t value = 0.299 * p.r + 0.587 * p.g + 0.114 * p.b;
...
void ImageTransform::randomModulation()
{
    ...
    for (uint32_t y = 0; y < cfg->h; y++)
    {
        for (uint32_t x = 0; x < cfg->w; x++)
        {
            ...
            int N = getRandomFromRange(-p.r,255-p.r);
...
void ImageTransform::errorDistribution()
{
    ...
    for (uint32_t y = 0; y < cfg->h; y++)
    {
        for (uint32_t x = 0; x < cfg->w; x++)
        {
            ...
            E = p.b - G;
            setPixel(x, y, G);
            if(y+1< cfg->h){ updatePixelWithError(x, y + 1, E * float(3.0 / 8.0)); }
            if (x + 1 < cfg->w && y + 1 < cfg->h) { updatePixelWithError(x + 1, y + 1, E * float(2.0 / 8.0)); }
            if (x + 1 < cfg->w) { updatePixelWithError(x + 1, y, E * float(3.0 / 8.0)); }
...
void ImageTransform::toneDependentErrorDistribution()
{
    ...
    for (uint32_t y = 0; y < cfg->h; y++)
    {
        for (uint32_t x = 0; x < cfg->w; x++)
        {
            ...
            if ((value > 0 && value < 40) || (value < 255 && value>215)) {
                if (x + 1 < cfg->w ) { updatePixelWithError(x + 1, y , E * float(7.0 / 16.0)); }
                if (x + 1 < cfg->w && y + 1 < cfg->h) { updatePixelWithError(x + 1, y + 1, E * float(1.0 / 16.0)); }
                if (y + 1 < cfg->h) { updatePixelWithError(x, y + 1, E * float(5.0 / 16.0)); }
                if (x - 1 < cfg->w && y + 1 < cfg->h) { updatePixelWithError(x - 1, y + 1, E * float(3.0 / 16.0)); }


            }
            if (value >= 40 && value <= 215) {
                if (x +1 < cfg->w && y < cfg->h) { updatePixelWithError(x+1, y, E * float(7.0 / 24.0)); }
                if (x +2< cfg->w && y < cfg->h) { updatePixelWithError(x+2, y, E * float(2.0 / 24.0)); }
                if (x-2 < cfg->w && y+1 < cfg->h) { updatePixelWithError(x-2, y+1, E * float(1.0 / 24.0)); }
                if (x-1 < cfg->w && y+1 < cfg->h) { updatePixelWithError(x-1, y+1, E * float(3.0 / 24.0)); }
                if (x < cfg->w && y+1 < cfg->h) { updatePixelWithError(x, y+1, E * float(5.0 / 24.0)); }
                if (x+1 < cfg->w && y+1 < cfg->h) { updatePixelWithError(x+1, y+1, E * float(1.0 / 24.0)); }
                if (x+2 < cfg->w && y+1 < cfg->h) { updatePixelWithError(x+2, y+1, E * float(0.5 / 24.0)); }
                if (x-2 < cfg->w && y+2 < cfg->h) { updatePixelWithError(x-2, y+2, E * float(1.0 / 24.0)); }
                if (x-1 < cfg->w && y+2 < cfg->h) { updatePixelWithError(x-1, y+2, E * float(1.0 / 24.0)); }
                if (x < cfg->w && y+2 < cfg->h) { updatePixelWithError(x, y+2, E * float(2.0 / 24.0)); }
                if (x+1 < cfg->w && y+2 < cfg->h) { updatePixelWithError(x+1, y+2, E * float(0.5 / 24.0)); }
